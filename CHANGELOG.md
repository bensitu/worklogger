...
- Settings -> AI now falls back to the persisted local model catalog when the remote catalog refresh fails, keeping cached downloaded/imported model entries manageable offline.
- Local Qwen3/Qwen3.5 generation now strips leaked thinking output more defensively and sends stronger no-thinking instructions for imported Qwen3-family GGUF models.
- Imported GGUF models now read context metadata from the file when available and derive safer max-output token and minimum-RAM values instead of showing fixed defaults.
- User-visible export, file-picker, analytics PDF, and timer labels now route through gettext catalogs.
- Deleting a pruned local/imported model now removes its card from the local-model selection list immediately.
- Local GGUF inference now caps the runtime llama.cpp context to a safe default while keeping the model's metadata context length, preventing high-context imports from failing to create `llama_context`.
- The benign llama.cpp `n_ctx_seq < n_ctx_train` capacity warning is now hidden during normal local-model runs while remaining available with `WORKLOGGER_LLAMA_VERBOSE=1`.

## [3.3.2] - 2026-05-08

### Added

- Added PyInstaller service-module hook coverage, strict internal import checks, and packaged `--smoke-import` validation.
...
- Fixed packaged executable validation for the report service import path.
- Limited ICS imports to 10 MiB files to avoid reading oversized calendar files into memory.
- External identity login now clears password-login failure rows after successful authentication.
- Machine-key fallback now uses a process-local ephemeral key if both keyring and fallback-file persistence fail.
...
- Added Linux release packaging through the root `WorkLogger_build_linux.sh` script and GitHub Actions artifact upload.
...
- PyInstaller packaging no longer bundles local or remote model catalog files; model metadata is refreshed from the GitHub-hosted `model_catalog.json` before opening the download UI.
